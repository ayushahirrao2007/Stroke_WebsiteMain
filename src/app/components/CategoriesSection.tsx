            ...
            >
              ...
              <div className="absolute inset-0 overflow-hidden">
                <img
                  ...
                  alt={category.title}
                  loading="lazy"
                  className={`w-full h-full object-cover transition-all duration-700 ${hoveredIndex === index ? 'scale-110 blur-sm' : 'scale-100'
                    }`}
                />
                <div className={`absolute inset-0 bg-gradient-to-br ${category.color} transition-opacity duration-500 ${hoveredIndex === index ? 'opacity-95' : 'opacity-85'
                  }`} />
              </div>
              ...
              <div className="relative p-4 sm:p-8 h-40 sm:h-48 flex flex-col justify-between">
                {/* Icon */}
                <div className={`${category.bgColor} rounded-xl sm:rounded-2xl w-12 h-12 sm:w-16 sm:h-16 flex items-center justify-center transform transition-all duration-500 ${hoveredIndex === index ? 'scale-110 rotate-12' : 'scale-100 rotate-0'
                  }`}>
                  <category.icon className="text-purple-700" size={24} />
                </div>

                {/* Text */}
                <div className={`transform transition-all duration-500 ${hoveredIndex === index ? 'translate-y-0 opacity-100' : 'translate-y-2 opacity-90'
                  }`}>
                ...
                {/* Hover Arrow */}
                <div className={`absolute bottom-6 right-6 transform transition-all duration-500 ${hoveredIndex === index ? 'translate-x-0 opacity-100' : 'translate-x-4 opacity-0'
                  }`}>
              ...
              {/* Ripple Effect */}
              <div className={`absolute inset-0 bg-white rounded-full transition-all duration-1000 ${hoveredIndex === index ? 'scale-150 opacity-0' : 'scale-0 opacity-30'
