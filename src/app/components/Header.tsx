...
type NavItem =
  ...
  | { type: 'button'; label: string; action: 'about' | 'types' };
...
const NAV_ITEMS: NavItem[] = [
  ...
  { type: 'button', label: 'Types of Strokes', action: 'types' },
...
export function Header() {
  ...
  /** Scrolls to the "Types of Strokes" section on the homepage */
  const handleTypesOfStrokes = useCallback(() => {
    setIsMenuOpen(false);
    if (location.pathname === '/') {
      document.getElementById('types-of-strokes')?.scrollIntoView({ behavior: 'smooth' });
    } else {
      navigate('/');
      setTimeout(() => {
        document.getElementById('types-of-strokes')?.scrollIntoView({ behavior: 'smooth' });
        ...
        <div className="max-w-[90rem] mx-auto px-4 sm:px-6 lg:px-10">
          <div className="flex items-center justify-between py-4" style={{ width: '100%' }}>
            ...
            <nav className="hidden lg:flex items-center gap-6">
              {NAV_ITEMS.map((item) => {
                if (item.type === 'button') {
                  const onClickHandler = item.action === 'about' ? handleAboutStroke : handleTypesOfStrokes;
                  return (
                    <button
                      key={item.label}
