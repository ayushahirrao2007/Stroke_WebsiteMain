...
export function ProgressDashboard() {
  const [animatedProgress, setAnimatedProgress] = useState<number[]>([0, 0]);
    ...
    <section className="bg-gradient-to-br from-purple-50 to-blue-50 py-12 sm:py-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-6 sm:mb-8">
          <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-purple-900 mb-2">Your Learning Progress</h2>
          <p className="text-sm sm:text-base text-purple-700">Track your journey to becoming a stroke care expert</p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 mb-8 sm:mb-12">
          {stats.map((stat, index) => (
            <div
              key={index}
              className="bg-white rounded-xl p-4 sm:p-6 shadow-lg hover:shadow-xl transition-all transform hover:-translate-y-1 cursor-pointer"
            >
              <div className={`bg-gradient-to-br ${stat.color} rounded-lg w-10 h-10 sm:w-12 sm:h-12 flex items-center justify-center mb-2 sm:mb-3`}>
                <stat.icon className="text-white" size={20} />
              </div>
              {/* Scale down stat value on mobile to prevent overflow in the 2-col grid */}
              <div className="text-2xl sm:text-3xl font-bold text-purple-900 mb-1">{stat.value}</div>
              <div className="text-xs sm:text-sm text-purple-600 leading-tight">{stat.label}</div>
            </div>
          ))}
        </div>
