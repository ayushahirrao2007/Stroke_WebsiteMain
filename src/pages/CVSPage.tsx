...
function getCVSSectionId(sectionId: number): string {
    return `cvs_section_${String(sectionId).padStart(2, '0')}`;
}
...
export default function CVSPage() {
    ...
    const { progress, markCVSComplete } = useUserProgress();
    ...
    const isSectionRead = (sectionId: number) => {
        const sid = getCVSSectionId(sectionId);
        return (progress.cvs || []).includes(sid) || readIds.has(sectionId);
    };
    ...
    const markRead = () => {
        ...
        const sid = getCVSSectionId(activeSection.id);
        markCVSComplete(sid);
