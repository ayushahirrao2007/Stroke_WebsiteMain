...
function getAnatomyTopicId(topicId: number): string {
  return `anatomy_topic_${String(topicId).padStart(2, '0')}`;
}

export function AnatomyPage() {
  const navigate = useNavigate();
  const { progress, markAnatomyComplete } = useUserProgress();
  ...
  const isTopicComplete = (topicId: number) => {
    const tid = getAnatomyTopicId(topicId);
    return (progress.anatomy || []).includes(tid) || completedIds.has(topicId);
  };

  const completedCount = topics.filter((t) => isTopicComplete(t.id)).length;
  const totalTopics = topics.length;
  const progressPercent = totalTopics > 0 ? Math.round((completedCount / totalTopics) * 100) : 0;

  const markComplete = () => {
    if (activeTopic) {
      const tid = getAnatomyTopicId(activeTopic.id);
      markAnatomyComplete(tid);
  ...
  const isActiveTopicComplete = activeTopic ? isTopicComplete(activeTopic.id) : false;
