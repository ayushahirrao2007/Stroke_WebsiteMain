...
import homeopathicMedicinesMd from '../data/Therapeutics/Homeopathic-Medicines-Stroke-Recovery.md?raw';
...
const LESSONS: TherapeuticsLesson[] = [
    ...
    {
        id: 4,
        title: 'Homeopathic Medicines Used in Stroke Recovery',
        markdown: homeopathicMedicinesMd,
        calloutHeading: '',
        pdfFilename: '',
    },
];
...
function getTherapeuticsLessonId(lessonId: number): string {
    return `therapeutics_lesson_${String(lessonId).padStart(2, '0')}`;
}
...
export default function TherapeuticsPage() {
    ...
    const { progress, markTherapeuticsComplete } = useUserProgress();
    ...
    const isLessonRead = (lessonId: number) => {
        const lid = getTherapeuticsLessonId(lessonId);
        return (progress.therapeutics || []).includes(lid) || readIds.has(lessonId);
    };

    const completedCount = LESSONS.filter(l => isLessonRead(l.id)).length;
    const totalLessons = LESSONS.length;
    const progressPercent = totalLessons
        ? Math.round((completedCount / totalLessons) * 100)
        : 0;

    const isActiveRead = isLessonRead(activeLesson.id);

    const markComplete = () => {
        const lid = getTherapeuticsLessonId(activeLesson.id);
        markTherapeuticsComplete(lid);
                ...
                <aside className="lg:w-80 xl:w-96 shrink-0 border-t lg:border-t-0 lg:border-r border-white/10">
                    ...
                    <nav className="p-2 space-y-1">
                        {LESSONS.map((lesson, index) => {
                            ...
                            const isDone = isLessonRead(lesson.id);
                            return (
                                <button
                                    key={lesson.id}
