...
function getAboutStrokeSectionId(sectionId: number): string {
    return `stroke_info_section_${String(sectionId).padStart(2, '0')}`;
}
...
export default function BrainStrokePage() {
    ...
    const { progress, markAboutStrokeComplete } = useUserProgress();
    ...
    const isSectionRead = (sectionId: number) => {
        const sid = getAboutStrokeSectionId(sectionId);
        return (progress.aboutStroke || []).includes(sid) || readIds.has(sectionId);
    };
    ...
    const markRead = () => {
        ...
        const sid = getAboutStrokeSectionId(activeSection.id);
        markAboutStrokeComplete(sid);
