...
import pharmaMd from '../data/Modern Medicines/Pharmacological-Management.md?raw';
...
/* ─────────────────────────────────────────────────────────────
   Parser — splits the Markdown file into logical sections
───────────────────────────────────────────────────────────── */
function parseIntoSections(raw: string): StrokeSection[] {
    ...
    for (const rawLine of rawLines) {
        ...
        if (isHeading) {
            if (current) {
                current.content = current.content.replace(/---\s*$/, '').trim();
...
function getModernMedicinesSectionId(sectionId: number): string {
    return `modern_medicines_section_${String(sectionId).padStart(2, '0')}`;
}

/* ─────────────────────────────────────────────────────────────
   Main Page Component
───────────────────────────────────────────────────────────── */
export default function ModernMedicinesPage() {
    const navigate = useNavigate();
    const { progress, markModernMedicinesComplete } = useUserProgress();
    ...
    useEffect(() => {
        try {
            const parsed = parseIntoSections(pharmaMd);
            setSections(parsed);
            if (parsed.length > 0) setActiveSection(parsed[0]);
        } catch (err) {
            console.error("Failed to load pharmacological management info:", err);
        }
    }, []);
    ...
    const isSectionRead = (sectionId: number) => {
        const sid = getModernMedicinesSectionId(sectionId);
        return (progress.modernMedicines || []).includes(sid) || readIds.has(sectionId);
    };

    const completedCount = sections.filter(s => isSectionRead(s.id)).length;
    const totalSections = sections.length;
    const progressPercent = totalSections
        ? Math.round((completedCount / totalSections) * 100)
        : 0;

    const isActiveRead = activeSection ? isSectionRead(activeSection.id) : false;

    const markRead = () => {
        if (!activeSection) return;
        const sid = getModernMedicinesSectionId(activeSection.id);
        markModernMedicinesComplete(sid);
    ...
    const SectionList = () => (
        <nav className="p-2 space-y-1" aria-label="Guide sections">
            {sections.map((section, index) => {
                ...
                const isDone = isSectionRead(section.id);
                return (
                    <button
                        key={section.id}
            ...
            <header className="relative z-10 flex items-center gap-3 px-4 sm:px-6 py-4 border-b border-white/10 backdrop-blur-sm">
                ...
                <div className="flex items-center gap-2 min-w-0 flex-1">
                    ...
                    <h1 className="text-white font-bold truncate text-sm sm:text-base">
                        Modern Medicines — Pharmacological Management
                    </h1>
                </div>
                {/* Progress pill */}
                                    ...
                                    <div className="prose prose-invert max-w-none
                                        ...
                                        <ReactMarkdown
                                            ...
                                            components={{
                                                table({ children }) {
                                                    return (
                                                        <div className="my-4 overflow-x-auto rounded-xl border border-white/15">
