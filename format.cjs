...
for (let i = 0; i < lines.length; i++) {
    ...
    if (i === 0 || trimmed === 'Brainstem Stroke') {
        newLines.push('# ' + trimmed + (raw.endsWith('\r') ? '\r' : ''));
        continue;
    }
    ...
    // check if it's already a heading somehow, or maybe the match replaces correctly
    if (syndromeMatch && !trimmed.startsWith('##') && !trimmed.startsWith('#')) {
        let replacement = '**' + syndromeMatch[1] + ':**';
        newLineStr = raw.replace(syndromeMatch[1] + ':', replacement);
    } else {
    ...
    if (trimmed.startsWith('-') && !newLineStr.trim().startsWith('-')) {
        newLines.push('- ' + newLineStr.trim() + (raw.endsWith('\r') ? '\r' : ''));
    } else {
